...
impl StubEmitter {
    ...
    fn emit_prologue_and_api_resolve(&mut self) {
        ...
        // VM metadata below r15 (r15 = [rbp-0x08]); must not alias r0..r15 at [rbp-0x80..-0x08].
        self.emit(&[0x48, 0xC7, 0x85, 0xA8, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00]); // call depth
        self.emit(&[0x48, 0xC7, 0x85, 0xB0, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00]); // push idx
    ...
    fn emit_handlers(&mut self) {
        ...
        self.emit(&[0x48, 0x89, 0x85, 0xB8, 0xFF, 0xFF, 0xFF]);
        self.jmp_to_dispatch();
        ...
        self.emit(&[0x48, 0x8B, 0x95, 0xA8, 0xFF, 0xFF, 0xFF]);
        self.emit(&[0x48, 0x89, 0xB4, 0xD5, 0x00, 0xFE, 0xFF, 0xFF]);
        self.emit(&[0x48, 0xFF, 0xC2]);
        self.emit(&[0x48, 0x89, 0x95, 0xA8, 0xFF, 0xFF, 0xFF]);
        self.lea_rip_rel32(0x48, 6, "bytecode");
        self.emit(&[0x48, 0x01, 0xF0]);
        self.emit(&[0x48, 0x89, 0xC6]);
        self.jmp_to_dispatch();
        ...
        self.label("h_ret");
        self.emit(&[0x48, 0x8B, 0x85, 0xA8, 0xFF, 0xFF, 0xFF]);
        self.emit(&[0x48, 0xFF, 0xC8]);
        self.emit(&[0x48, 0x89, 0x85, 0xA8, 0xFF, 0xFF, 0xFF]);
        ...
        self.emit(&[0x48, 0xFF, 0xC6]);
        self.emit(&[0x48, 0x8B, 0x44, 0xCD, 0x80]);
        self.emit(&[0x48, 0x8B, 0x95, 0xB0, 0xFF, 0xFF, 0xFF]);
        self.emit(&[0x48, 0x89, 0x84, 0xD5, 0x80, 0xFD, 0xFF, 0xFF]);
        self.emit(&[0x48, 0xFF, 0xC2]);
        self.emit(&[0x48, 0x89, 0x95, 0xB0, 0xFF, 0xFF, 0xFF]);
        self.jmp_to_dispatch();
    ...
    fn emit_push_flags_and_jcc(&mut self, jcc: u8) {
        self.emit(&[0xFF, 0xB5, 0xB8, 0xFF, 0xFF, 0xFF]);
        self.emit(&[0x9D]);
    ...
    fn emit_native_call_handler(&mut self) {
        self.emit(&[0x48, 0x8B, 0x06]);
        self.emit(&[0x48, 0x83, 0xC6, 0x08]);
        // Save bytecode pointer past func id (dedicated slot, not VM r3 at [rbp-0x68]).
        self.emit(&[0x48, 0x89, 0xB5, 0xC0, 0xFF, 0xFF, 0xFF]);
        ...
        self.label("nc_done");
        self.emit(&[0x48, 0x8B, 0xB5, 0xC0, 0xFF, 0xFF, 0xFF]);
    }
...
mod tests {
    ...
    fn vm_metadata_slots_do_not_alias_vm_registers() {
        let (stub, _) = create_vm_interpreter_stub(0, 0);
        // VM reg n lives at [rbp + n*8 - 0x80]. Metadata must sit below r0 (rbp-0x80).
        let call_depth_init = [0x48u8, 0xC7, 0x85, 0xA8, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00];
        let push_idx_init = [0x48u8, 0xC7, 0x85, 0xB0, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00];
        assert!(
            stub.windows(call_depth_init.len()).any(|w| w == call_depth_init),
            "call depth must use [rbp-0xA8], not VM r9 at [rbp-0x38]"
        );
        assert!(
            stub.windows(push_idx_init.len()).any(|w| w == push_idx_init),
            "push stack index must use [rbp-0xB0], not VM r13 at [rbp-0x18]"
        );
        let flags_store = [0x48u8, 0x89, 0x85, 0xB8, 0xFF, 0xFF, 0xFF];
        assert!(
            stub.windows(flags_store.len()).any(|w| w == flags_store),
            "cmp flags must use [rbp-0xB8], not VM r2 at [rbp-0x70]"
        );
        let rsi_save = [0x48u8, 0x89, 0xB5, 0xC0, 0xFF, 0xFF, 0xFF];
        assert!(
            stub.windows(rsi_save.len()).any(|w| w == rsi_save),
            "native_call resume must use [rbp-0xC0], not VM r3 at [rbp-0x68]"
        );
    }
