...
pub enum X64InstrKind {
    ...
    /// `call [rip+disp]` — typical MinGW IAT thunk pattern.
    CallIndRip { disp: i32 },
    Ret,
    Push { reg: X64Reg },
    Pop { reg: X64Reg },
...
fn instruction_at_offset(instrs: &[X64Instruction], offset: usize) -> Option<&X64Instruction> {
    if let Some(exact) = instrs.iter().find(|i| i.offset == offset) {
        return Some(exact);
    }
    if let Some(containing) = instrs
        .iter()
        .find(|i| i.offset < offset && offset < i.offset + i.bytes.len())
    {
...
fn cmp_operand_reg(kind: &X64InstrKind) -> Option<X64Reg> {
    match kind {
        X64InstrKind::CmpRegReg { reg2, .. } => Some(*reg2),
        X64InstrKind::CmpRegImm { reg, .. } => Some(*reg),
        _ => None,
    }
}

fn cmp_or_feed_imm_before(instrs: &[X64Instruction], jcc_offset: usize) -> usize {
    let Some(cmp) = instrs
        .iter()
        .filter(|i| i.offset < jcc_offset)
        .max_by_key(|i| i.offset)
    else {
        return jcc_offset;
    };
    ...
    if let Some(feed_reg) = cmp_operand_reg(&cmp.kind) {
        if let Some(prev) = instrs
            .iter()
            .filter(|i| i.offset < cmp.offset)
            .max_by_key(|i| i.offset)
        {
            if let X64InstrKind::MovRegImm { reg, imm } = &prev.kind {
                if *reg == feed_reg && *imm <= 3 {
                    return prev.offset;
                }
            }
        }
...
fn retarget_unconditional_jmp_from_jcc(instrs: &[X64Instruction], target_x64_offset: usize) -> usize {
    let Some(instr) = instruction_at_offset(instrs, target_x64_offset) else {
        return target_x64_offset;
    };
    if !is_conditional_branch_kind(&instr.kind) {
        return target_x64_offset;
    }
    cmp_or_feed_imm_before(instrs, instr.offset)
}
...
fn is_mul_or_add_kind(kind: &X64InstrKind) -> bool {
    matches!(
        kind,
        X64InstrKind::ImulRegReg { .. }
            | X64InstrKind::ImulRegMem { .. }
            | X64InstrKind::AddRegReg { .. }
    )
}
...
fn mov_before_mul_or_add(instrs: &[X64Instruction], mul_offset: usize) -> Option<usize> {
    let mul = instrs.iter().find(|i| i.offset == mul_offset)?;
...
fn retarget_conditional_jmp_from_mul(instrs: &[X64Instruction], target_x64_offset: usize) -> usize {
    let Some(instr) = instruction_at_offset(instrs, target_x64_offset) else {
        return target_x64_offset;
    };
    mov_before_mul_or_add(instrs, instr.offset).unwrap_or(target_x64_offset)
}

fn resolve_conditional_jump_target(
    instrs: &[X64Instruction],
    label_map: &std::collections::HashMap<usize, usize>,
    target_x64_offset: usize,
) -> Option<usize> {
...
fn emit_add_reg_reg_x64(bytecode: &mut Vec<u8>, dst: &X64Reg, src: &X64Reg) {
    emit_add_reg_reg(bytecode, dst.to_vm_reg(), src.to_vm_reg());
}
...
fn is_crt_external_instr(pe: &PEFile, imports: &ImportTable, instr: &X64Instruction) -> bool {
    let instr_rva = match pe.file_offset_to_rva(instr.offset) {
        Ok(r) => r,
        Err(_) => return false,
    };
    let target_rva = match &instr.kind {
        ...
        X64InstrKind::Call { target_offset } => {
            let target_off =
                (instr.offset as i32 + instr.bytes.len() as i32 + *target_offset) as usize;
            match pe.file_offset_to_rva(target_off) {
                Ok(r) => r,
                Err(_) => return false,
            }
        }
    ...
    if let Some(entry) = pe.resolve_iat_target(imports, target_rva) {
        return imports.is_crt_startup(entry);
    }
    // Direct rel32 calls outside lifted region with CRT-like names are rare; skip by name heuristic.
    false
}
...
fn fuse_load_operand(
    kind: &X64InstrKind,
    stack_map: &mut std::collections::HashMap<i32, u8>,
    next_stack_reg: &mut u8,
) -> Option<(X64Reg, u8)> {
    match kind {
        X64InstrKind::MovRegReg { dst, src } => Some((*dst, src.to_vm_reg())),
        X64InstrKind::MovRegMem {
            dst,
            base,
            offset,
...
fn try_fuse_index_base_mov_pair(
    ...
    let mut after_pos = next_pos + 1;
    while after_pos < lift_indices.len() {
        if is_fuse_gap_skippable(&instrs[lift_indices[after_pos]].kind) {
            after_pos += 1;
            continue;
        }
        break;
    }
    if after_pos < lift_indices.len() {
        if let X64InstrKind::AddRegReg { dst: add_dst, .. } = &instrs[lift_indices[after_pos]].kind
        {
            if add_dst.to_vm_reg() == dst.to_vm_reg() {
                end_pos = after_pos;
            }
        }
    }
    ...
    emit_add_three(bytecode, dst_vm, dst_vm, index_vm);
    Some(end_pos)
}
...
/// Skip MinGW printf-stub shuffles that clobber the saved integer before we lower the call to nc2.
...
pub fn lift_to_vm_bytecode(instrs: &[X64Instruction], _base_rva: u32) -> Vec<u8> {
    let (bytecode, _) = lift_to_vm_bytecode_internal(instrs, _base_rva, false, 0);
    bytecode
}
...
pub fn lift_to_vm_bytecode_with_map(
    ...
    lift_to_vm_bytecode_internal(instrs, base_rva, false, 0)
}
...
pub fn lift_to_vm_bytecode_for_main(
    ...
    if let Some(string_bytes) = printf_literal {
        ...
        let mut embed = string_bytes.to_vec();
        if !embed.ends_with(&[0]) {
            embed.push(0);
        }
        bytecode.extend_from_slice(&embed);
...
fn emit_transition_refresh(
    ...
    if let Some(tx_id) = block_plan.transition_for_edge(pred_bb_id, bb_id) {
        emit_block_map_refresh(bytecode, tx_id);
        set_active_map(&block_plan.map_for_tx_or_base(tx_id, base_map));
    } else {
        // Linear lift may touch blocks only reached via VM jump retargeting (no CFG edge).
        emit_block_map_refresh(bytecode, bb_id);
        set_active_map(&BlockMapPlan::block_opcode_map(base_map.seed(), bb_id as usize));
    }
}
...
fn edge_pad_for_target(
    ...
    bytecode.extend_from_slice(&(body as u64).to_le_bytes());
    ...
    for instr in instrs {
        ...
        match &instr.kind {
            ...
            X64InstrKind::AddRegReg { dst, src } => {
                emit_add_reg_reg_x64(&mut bytecode, dst, src);
            }
            ...
            X64InstrKind::SubMemImm { base, offset, imm } => {
                if *base == X64Reg::Rbp || *base == X64Reg::Ebp {
                    ...
                    bytecode.push(15);
                    bytecode.extend_from_slice(&(*imm as u64).to_le_bytes());
                    emit_sub_three(&mut bytecode, stack_reg, stack_reg, 15, pack_seed);
                }
            }
            X64InstrKind::AddRegImm { reg, imm } => {
                emit_add_reg_imm(&mut bytecode, reg.to_vm_reg(), *imm as u64);
            }
            X64InstrKind::AddMemImm { base, offset, imm } => {
                if *base == X64Reg::Rbp || *base == X64Reg::Ebp {
                    ...
                    emit_add_reg_imm(&mut bytecode, stack_reg, *imm as u64);
                }
            }
            ...
            X64InstrKind::Inc { reg } => {
                emit_add_reg_imm(&mut bytecode, reg.to_vm_reg(), 1);
            }
            ...
            X64InstrKind::LeaRegReg { dst, base, index } => {
                if dst == base {
                    emit_add_reg_reg_x64(&mut bytecode, dst, index);
                } else {
                    bytecode.push(active_encode(OpCode::Move));
                    bytecode.push(dst.to_vm_reg());
            ...
            X64InstrKind::MovzxByteRegReg { dst, base, index } => {
                ...
                emit_add_reg_reg_x64(&mut bytecode, &addr_reg, index);
                bytecode.push(active_encode(OpCode::LoadByte));
                bytecode.push(dst.to_vm_reg());
    ...
    for (pos, &idx) in lift_indices.iter().enumerate() {
        ...
        if instr.offset < main_x64_offset {
            ...
            if vm_bb && !emitted_block_map.contains(&bb.id) {
                ...
                if instr.offset == bb.start {
                    label_map.insert(bb.start, bytecode.len());
                }
    ...
    for (placeholder_pos, target_x64_offset, is_unconditional, source_x64_offset) in pending_jumps {
        ...
        if let Some(mut target_vm_offset) = target_vm_offset {
            ...
            if let (Some(pred_bb), Some(succ_bb)) = (
                ...
                bb_for_offset(&main_blocks, target_x64_offset).map(|b| b.id as u16),
            ) {
                if incoming.get(&succ_bb).copied().unwrap_or(1) > 1 {
                    target_vm_offset = edge_pad_for_target(
                        &mut bytecode,
                        block_plan,
                        opcode_map,
                        &mut edge_pads,
...
mod tests {
    ...
    fn puts_iat_call_emits_iat_native_call_id() {
        ...
        let text = pe.get_section(".text").unwrap();
        let main_off = pe.rva_to_file_offset(text.virtual_address).unwrap();
        let code = &pe.data[main_off..main_off + 64];
        let instrs = disassemble_cfg_function(code, main_off);
        assert!(
    ...
    fn mov_rr(offset: usize, dst: X64Reg, src: X64Reg) -> X64Instruction {
        X64Instruction {
            offset,
            bytes: vec![0x48, 0x89, 0xC0],
            kind: X64InstrKind::MovRegReg { dst, src },
        }
    }
    ...
    fn mingw_printf_stub_restores_int_to_r2_before_nc2() {
        ...
        let instrs = vec![
            ...
            mov_rr(stub + 16, X64Reg::Rax, X64Reg::R14),
            mov_rr(stub + 19, X64Reg::Rdx, X64Reg::Rax),
            call_at(stub + 22, 0x1000),
            ret_at(stub + 27),
            X64Instruction {
                offset: main,
            ...
            mov_rr(main + 8, X64Reg::Rcx, X64Reg::Rax),
            X64Instruction {
                offset: main + 11,
    ...
    #[test]
    fn unconditional_jmp_one_byte_into_jcc_retargets_to_cmp() {
        let main_off = 0x400;
        let cmp_off = main_off + 0x10;
        let jle_off = cmp_off + 3;
        let instrs = vec![
            X64Instruction {
                ...
                bytes: vec![0xE9, 0x00, 0x00, 0x00, 0x00],
                kind: X64InstrKind::Jmp {
                    target_offset: (jle_off as i32 + 1) - (main_off as i32 + 5),
                },
            },
            X64Instruction {
                offset: cmp_off,
    ...
    #[test]
    fn conditional_jmp_to_mul_retargets_to_preceding_mov() {
        let main_off = 0x400;
        let mov_off = main_off + 0x39;
        let mul_off = mov_off + 3;
        let cmp_off = main_off + 0x18d;
        let jle_off = cmp_off + 3;
        let instrs = vec![
            X64Instruction {
                offset: mov_off,
                bytes: vec![0x4C, 0x89, 0xD0],
                kind: X64InstrKind::MovRegReg {
                    dst: X64Reg::Rax,
                    src: X64Reg::R10,
                },
            ...
            X64Instruction {
                offset: mul_off,
                bytes: vec![0x4D, 0x0F, 0xAF, 0xC3],
                kind: X64InstrKind::ImulRegReg {
                    dst: X64Reg::Rax,
                    src: X64Reg::R11,
                },
            },
    ...
    #[test]
    fn conditional_jmp_one_byte_into_mul_retargets_to_mov() {
        let main_off = 0x400;
        let mov_off = main_off + 0x39;
        let mul_off = mov_off + 3;
        let jle_off = main_off + 0x190;
        let instrs = vec![
            ...
            X64Instruction {
                ...
                bytes: vec![0x7E, 0x00],
                kind: X64InstrKind::Jle {
                    target_offset: (mul_off as i32 + 1) - (jle_off as i32 + 2),
                },
            },
            ret_at(jle_off + 2),
        ];
    ...
    #[test]
    fn fuses_stack_load_pair_and_skips_redundant_add() {
        let main_off = 0x400;
        let instrs = vec![
            X64Instruction {
                offset: main_off,
            ...
            X64Instruction {
                offset: main_off + 8,
                bytes: vec![0x48, 0x01, 0xD0],
                kind: X64InstrKind::AddRegReg {
                    dst: X64Reg::Rax,
                    src: X64Reg::Rdx,
                },
            },
